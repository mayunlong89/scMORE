...
getModuleScore <- function(eachModule) {
  ...
  filtered_eachModule <- eachModule %>%
    group_by(Target) %>%
    slice_max(Importance_weighted, with_ties = FALSE) %>% # Ensure only the maximum value per Target is kept
    ungroup()
  ...
  eachModule_Importance_score <- target_importance %>%
    add_row(Target = unique(filtered_eachModule$TF), Importance_weighted = tf_importance, .before = 1)
