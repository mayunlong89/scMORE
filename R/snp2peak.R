...
snp2peak <- function(snp_info, peak2gene_strength, buffer = 50) {
  ...
  # Create Peak GenomicRanges object
    ...
    # Extend peak ranges
    if (buffer > 0) {
      extended_peak_ranges <- resize(peak_ranges, width = width(peak_ranges) + buffer * 2, fix = "center")
      start(extended_peak_ranges) <- pmax(start(extended_peak_ranges), 1) # Ensure ranges start >= 1
    } else {
      extended_peak_ranges <- peak_ranges
    }
    mcols(extended_peak_ranges) <- mcols(peak_ranges)
    ...
    overlap_results <- data.frame(
      ...
      peak_chr = seqnames(extended_peak_ranges)[subjectHits(overlaps)],
      peak_start = start(extended_peak_ranges)[subjectHits(overlaps)],
      peak_end = end(extended_peak_ranges)[subjectHits(overlaps)],
      peak_ids = mcols(extended_peak_ranges)$peak_ids[subjectHits(overlaps)],
      TF = mcols(extended_peak_ranges)$TF[subjectHits(overlaps)],
      Target = mcols(extended_peak_ranges)$Target[subjectHits(overlaps)],
      Strength = mcols(extended_peak_ranges)$Strength[subjectHits(overlaps)],
      Significance = mcols(snp_ranges)$log_P[queryHits(overlaps)]
    )

    # Step 5: Ensure all peaks are included
    all_peaks <- data.frame(
      peak_chr = seqnames(extended_peak_ranges),
      peak_start = start(extended_peak_ranges),
      peak_end = end(extended_peak_ranges),
      peak_ids = mcols(extended_peak_ranges)$peak_ids,
      TF = mcols(extended_peak_ranges)$TF,
      Target = mcols(extended_peak_ranges)$Target,
      Strength = mcols(extended_peak_ranges)$Strength
    )
